...
function calculateAverageMetrics(testCase: any) {
  return {
    ...
    data: [{
      ...
      intersystems: Math.round(testCase.data.reduce((sum: number, dp: any) => sum + dp.intersystems, 0) / testCase.data.length)
    }]
  }
}
...
export function Suite({ suite }: { suite: BenchmarkSuite }) {
  ...
  const hasValidData = suite.result?.data?.some((dataPoint: any) => 
    dataPoint.aidbox !== 0 || dataPoint.medplum !== 0 || dataPoint.hapi !== 0 || dataPoint.microsoft !== 0 || dataPoint.wso2 !== 0 || dataPoint.intersystems !== 0
  );
